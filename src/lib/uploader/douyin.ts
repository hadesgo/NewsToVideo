...
export const douyinSetup = async (accountFile: string, handle = false) => {
  if (!fs.existsSync(accountFile) || !(await cookieAuth(accountFile))) {
    ...
    logger.info(
      "[抖音] [+] cookie文件不存在或已失效，即将自动打开浏览器，请扫码登录，登陆后会自动生成cookie文件"
    );
...
export class DouYinVideo {
  ...
  async upload() {
    ...
    if (await page.locator(thirdPartElement).count()) {
      // 检测是否是已选中状态
      const selectItems = (await page.$eval(
        thirdPartElement,
        (div) => div.className
      )) as string[];
      if (!selectItems.includes("semi-switch-checked")) {
        await page
          .locator(thirdPartElement)
  ...
  async setThumbnail(page: Page, thumbnailPath: string) {
    if (thumbnailPath) {
      ...
      await page.waitForSelector("div[class*='modal']:visible", {
        timeout: 60000,
      });
