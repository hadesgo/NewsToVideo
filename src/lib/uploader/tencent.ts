...
const formatStrForShortTitle = (originTitle: string) => {
  ...
  if (formattedString.length > 16) {
    ...
    formattedString =
      formattedString +
      Array(6 - formattedString.length)
        .fill(" ")
        .join("");
  }
...
const cookieAuth = async (accountFile: string) => {
  ...
  try {
    await page.waitForSelector('div.title-name:has-text("微信小店")', {
      timeout: 5000,
    }); // 等待5秒
...
export class TencentVideo {
  ...
  async clickPublish(page: Page) {
    while (true) {
      try {
        const publishButtion = page.locator(
  ...
  async detectUploadStatus(page: Page) {
    while (true) {
      ...
      try {
        // 匹配删除按钮，代表视频上传完毕
        const publishButtion = await page
  ...
  async addOriginal(page: Page) {
    ...
    if (
      ...
      if (
        ...
        await page
          .locator(
            `'div.form-content:visible ul.weui-desktop-dropdown__list li.weui-desktop-dropdown__list-ele:has-text("${
              this.#category
            }")`
          )
          .first()
