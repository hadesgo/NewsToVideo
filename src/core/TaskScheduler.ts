...
export class TaskScheduler {
   ...
   * 注册任务
   */
  public registerTask(config: TaskConfig, taskFunction: TaskFunction): void {
  ...
  public startTask(taskId: string): void {
    ...
    logger.info(
      `Task started: ${config.name} (${config.id}) with cron: ${config.cronExpression}`
    );
  }
  ...
  public stopTask(taskId: string): void {
    ...
    if (taskInfo && taskInfo.status === TaskStatus.RUNNING) {
      ...
      if (taskInfo.startTime) {
        taskInfo.duration =
          taskInfo.endTime.getTime() - taskInfo.startTime.getTime();
      }
    }
  ...
  private async executeTask(
    ...
    logger.info(
      `Task started: ${
        config.name
      } (${taskId}) at ${context.triggeredAt.toISOString()}`
    );
    ...
    try {
      ...
      const result = await ErrorHandler.executeWithRetry(
        ...
        {
          maxAttempts:
            config.maxRetries || configManager.getAppConfig().maxRetryCount,
      ...
      logger.info(
        `Task completed: ${config.name} (${taskId}) in ${taskInfo.duration}ms`
      );
      return result;
    } catch (error) {
      // 任务执行失败
      ...
      logger.error(
        `Task failed: ${config.name} (${taskId}) after ${taskInfo.duration}ms - ${taskInfo.error}`
      );
  ...
  public getTaskStats(): {
    ...
    return {
      total: infos.length,
      running: infos.filter((info) => info.status === TaskStatus.RUNNING)
        .length,
      completed: infos.filter((info) => info.status === TaskStatus.COMPLETED)
        .length,
      failed: infos.filter((info) => info.status === TaskStatus.FAILED).length,
      pending: infos.filter((info) => info.status === TaskStatus.PENDING)
        .length,
    };
  }
  ...
  public cleanupCompletedTasks(olderThanHours: number = 24): void {
    ...
    logger.info(
      `Cleaned up completed tasks older than ${olderThanHours} hours`
    );
  }
}
