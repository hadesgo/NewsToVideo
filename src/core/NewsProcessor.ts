...
export class NewsProcessor {
      ...
      async () => {
        ...
        if (!audioInfo.audio || !fs.existsSync(audioInfo.audio)) {
          throw new AppError(
            ...
            ErrorHandler.createContext("news-processor", "generate-audio", {
              index,
  ...
  public async getMaterial(
    ...
    return ErrorHandler.executeWithRetry(
      async () => {
        ...
        // 先尝试获取视频素材
        try {
          const videoPath = await getVideo(
          ...
          logger.warn(
            `Failed to get video material for index ${index}, falling back to image: ${error}`
          );
        }
        ...
        if (!fs.existsSync(imagePath)) {
          throw new AppError(
            ...
            ErrorHandler.createContext("news-processor", "get-material", {
              index,
  ...
  public async generateTalkVideo(
    ...
    return ErrorHandler.executeWithRetry(
      ...
      ErrorHandler.createContext("news-processor", "generate-talk-video", {
        index,
        audioPath,
      }),
  ...
  public async processNewsForLayout(
    ...
    logger.info(
      `Processing ${newsList.length} news items for layout: ${layout}`
    );
    ...
    for (let i = 0; i < newsList.length; i++) {
      ...
      // 生成音频（如果尚未生成）
      if (!layer.audio?.path) {
        const audioInfo = await this.generateAudio(
          newsItem,
          i,
          progressCallback
        );
      ...
      if (!layer.material?.path) {
        const materialInfo = await this.getMaterial(
          newsItem,
          layout,
          layer.duration,
          i,
      ...
      if (!layer.talkVideo?.path) {
        const talkVideoInfo = await this.generateTalkVideo(
          layer.audio.path,
          i,
          progressCallback
        );
      ...
      logger.info(
        `Completed processing news ${i + 1}/${newsList.length}: ${
          newsItem.title
        }`
      );
    }
   ...
   * 根据布局获取分辨率
   */
  private getResolutionForLayout(layout: LayoutType): {
    width: number;
    height: number;
  } {
    switch (layout) {
   ...
   * 处理所有新闻
   */
  public async processAllNews(progressCallback?: ProgressCallback): Promise<{
    newsList: NewsData[];
    configs: Map<LayoutType, NewsVideoConfig>;
  }> {
    this.ensureDirectories();
    ...
    for (const layout of layouts) {
      ...
      try {
        const config = await this.processNewsForLayout(
          newsList,
          layout,
          progressCallback
        );
        ...
        await ErrorHandler.handle(
          error,
          ErrorHandler.createContext("news-processor", "process-layout", {
            layout,
          })
        );
        throw error;
