...
export class ErrorHandler {
  ...
  private static analyzeGenericError(error: Error, context: ErrorContext) {
    ...
    if (context.operation.includes('video') || context.operation.includes('audio')) {
      return {
   ...
   * 记录错误日志
   */
  private static async logError(errorInfo: any, context: ErrorContext): Promise<void> {
    const logData = {
   ...
   * 发送错误通知
   */
  private static async sendErrorNotification(errorInfo: any, context: ErrorContext): Promise<void> {
    try {
  ...
  static async executeWithRetry<T>(
    ...
    for (let attempt = 1; attempt <= config.maxAttempts; attempt++) {
      try {
        ...
        logger.info(`Retrying operation after ${delay}ms (attempt ${attempt + 1}/${config.maxAttempts})`)
