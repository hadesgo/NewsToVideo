...
export class VideoGenerator {
    ...
    for (let i = 0; i < this.config.layers.length; i++) {
      ...
      if (!layer.material?.path || !fs.existsSync(layer.material.path)) {
        throw new AppError(
          ...
          ErrorHandler.createContext("video-generator", "validate-config", {
            layerIndex: i,
            materialPath: layer.material?.path,
          })
        );
      }
      ...
      if (!layer.talkVideo?.path || !fs.existsSync(layer.talkVideo.path)) {
        throw new AppError(
          ...
          ErrorHandler.createContext("video-generator", "validate-config", {
            layerIndex: i,
            talkVideoPath: layer.talkVideo?.path,
          })
        );
      }
    ...
    for (const asset of requiredAssets) {
      if (!fs.existsSync(asset)) {
        throw new AppError(
          ...
          ErrorHandler.createContext("video-generator", "check-dependencies", {
            assetPath: asset,
          })
        );
      }
    ...
    return ErrorHandler.executeWithRetry(
      async () => {
        ...
        if (fs.existsSync(this.outputPath)) {
          logger.warn(
            `Output file already exists, removing: ${this.outputPath}`
          );
  ...
  private calculateTotalDuration(): number {
    ...
    const contentDuration = this.config.layers.reduce(
      (total, layer) => total + layer.duration,
      0
    );
  ...
  public getVideoInfo(): {
    ...
    const estimatedBitrate =
      this.config.layout === "portrait" ? 2000000 : 4000000; // 2Mbps for portrait, 4Mbps for landscape
